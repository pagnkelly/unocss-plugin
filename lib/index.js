    ...
    compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
      compilation.hooks.processAssets.tapPromise({
        ...
        const processStyle = async (file, source) => {
          ...
          const unores = await uno.generate(content, { preflights: false, safelist: false, minify: true })
        ...
        // 生成主包windi.css
        let mainWindiFile
        const mainWindiFileContent = await this.generateStyle(uno, mainClassesMap, { safelist: false })
        ...
        await Promise.all(Object.entries(packageClassesMaps).map(async ([packageRoot, classesMap]) => {
          let windiFile
          const windiFileContent = await this.generateStyle(uno, classesMap, { preflights: false, safelist: false, minify: true })
          if (windiFileContent) {
