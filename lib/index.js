...
class MpxUnocssPlugin {
  ...
  async generateStyle(uno, classesMap = {}, preflightOptions = {}) {
    ...
    classes.forEach((item) => {
      const selector = core.toEscapedSelector(item)
  ...
  apply(compiler) {
    ...
    compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
      compilation.hooks.processAssets.tapPromise({
        ...
        // 生成主包windi.css
        let mainWindiFile
        const mainWindiFileContent = await this.generateStyle(uno, mainClassesMap)
        console.log(mainWindiFileContent, 'mainWindiFileContent')
